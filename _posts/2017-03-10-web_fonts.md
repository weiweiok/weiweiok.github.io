...
第 3 条，偶尔也有例外，比如[这个例子][5]，台湾繁体中文的 Win 和 Mac 系统中，都有一个中文名为 “標楷體” 的字体，但在2个系统中，对应的分别是两个不同的英文字体名 “DFKai-SB” 和 “BiauKai” ，这时若只写其中一个英文名，另一个系统则无法显示，反而只写中文名后，两个系统均正常。

第 4 条，就是上例问题的完美解决方案，当然也可简单解决：2个英文名都写。

第 5 条，不同的系统差异、同系统之间的版本差异、一些软件自行安装字体导致系统字体列表杂乱，难以辨识哪些字体安全可用，于是引入下面话题。

### Web 安全字体

 通过对比 [Windows 默认字体列表][1] 和 [OS X 默认字体列表][2]，以及这些字体 [在各系统中版本占有率][3]，接近绝对安全的（字体名称和样子一致占有率95%+），有以下英文字体：
...
题外话：Hiragino Sans GB 由日本 JIYUKOBO Ltd. 公司和汉仪联合开发，是第一个符合 GB 18030 标准的日本出品字体，内含简繁全套汉字。而在 Linux 系统，常用的则是开源的[文泉驿系列][6]字体。
...
实现字体格式转换的软件有很多（如：[gulp][36] ，及下文提到的其他工具），最简单的是通过一个在线工具 [fontsquirrel][8] ，操作步骤见下图：
...
然后第二种，就涉及到自行实现 Web-Fonts 整体逻辑。2014 年，国内大牛 [@糖饼][26] 借助一系列开源项目（[css.js][27]、[jsdom][28]、[font-optimizer][29]、[ttf2eot][30]、[ttf2woff][31]、[ttf2svg][32]），实现了 Demo ，并以 “[字蛛 font-spider][33]” 项目开源发布。

![](http://imglf1.nosdn.127.net/img/MzVqS0VIeGlFdmxTbXFmN3ZPWm9JbXpPenBRN0MxZUx0NHl6bEk3TjQ3NGZGdDIvTzhhOEFRPT0.png)

字蛛 font-spider 项目：[官网][33]，[源码][34]，[诞生过程][35]，感谢这些前浪们。
...
SVG 转字体的工具，上文提到过，但在线的 [fontsquirrel][8] 只能单纯实现格式转换，总不能一个字体文件里就一个字吧，如何在一个字体文件中包含任意数量文字呢，上文提到的专业字体编辑软件自然可以，只是大材小用且略麻烦，好在也有在线工具可以做到，如：[fontello][43] 和 [icomoon][44]，还有阿里的图标综合站 [iconfont][45] 。
