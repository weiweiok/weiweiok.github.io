...
于是在 Web 中常见的4种字体格式中，woff 体积最小除 IE8 外浏览器全都支持，因此一般只需使用 woff 格式即可，处女座可以考虑联合 eot、svg、ttf 一起使用。

### 字体格式之间的转换

实现字体格式转换的软件有很多（如：[gulp][36] 及下文提到的其他工具），最简单的是通过一个在线工具 [fontsquirrel.com][8] ，操作步骤见下图：
...
简单点说，就是提前整理出一个页面中所有用到所有汉字，然后生成一个只包含这些汉字的字体文件，然后再通过 CSS 加载，这个叫“子集化”，动态子集化就是以上全部动态实现。
...
一种是通过前端方式，往页面中添加一段 JS 代码，由 JS 自行扫描页面 DOM ，把文字汇总后传回自己的云服务器，再由于云服务器动态生成字体，返回字体的 URL。另一种是通过服务端语言，用户在服务端自己整理需生成的文字，再发送给云 API ，并拿到字体的 URL。
